...
/* 构造函数&析构函数                                                    */
/************************************************************************/
TaskStationMerge::TaskStationMerge()
{
	LOG_(LOGID_DEBUG, LOG_F("Entry TaskStationMerge()"));
	// 初始化状态为僵死，不初始化不予执行任务
	m_strTaskName = "站点合并处理任务";

	m_emTaskState = _TASK_DEAD;
	m_pCachedImpl = NULL;
	m_strClearHasFlg = "";
	LOG_(LOGID_DEBUG, LOG_F("Leave TaskStationMerge()"));
}
...
/* 设置所管理的产品                                                     */
/************************************************************************/
void TaskStationMerge::Init(ST_STATION_CFG stStationCfg, ST_CACHED stCached)
{
...
/* 初始化缓存数据                                                       */
/************************************************************************/
bool TaskStationMerge::InitCached()
{
	// 建立缓存连接
...
/* 执行产品扫描处理任务                                                 */
/************************************************************************/
bool TaskStationMerge::ExecTask()
{
	// 数据保存处理
	if (m_stStationCfg.lstReportTimes.size() != 0)
	{
		// 保存文件功能没有禁用，则进行处理
		MergeProcess();
	}
...
/* 执行扫描处理                                                         */
/************************************************************************/
bool TaskStationMerge::RawProcess()
{
...
/* 获取产品文件列表                                                     */
/************************************************************************/
bool TaskStationMerge::GetProductFileList(QString strSrcFolder, QString strSrcFile, ST_FILE_FORMAT stFileNameFormat, QFileInfoList& lstFile)
{
	QDir dDir;
	// 检验产品目录是否存在
...
/* 存储数据到缓存中                                                     */
/************************************************************************/
int TaskStationMerge::SaveData2Cached(ST_SRC_ITEM stSrcItem, int nYear, int nMonth, int nDay, int nHour, st_station * pStation, int nStationCnt)
{
	// 检查缓存连接
	if (m_pCachedImpl == NULL)
	{
...
/* 保存数据处理                                                         */
/************************************************************************/
bool TaskStationMerge::MergeProcess()
{
	int nYear, nMonth, nDay;
	// 当前日期
	QDateTime tToday = QDateTime::currentDateTime();
	...
	for (iter = m_stStationCfg.lstReportTimes.begin(); iter != m_stStationCfg.lstReportTimes.end(); iter++)
	{
		...
		if (nErr == SUCCESS)
		{
			...
			LOG_(LOGID_STATION, LOG_F("[%s-%d][%s]时次站点数据处理成功."), LOG_STR(m_stStationCfg.strCityType), m_stStationCfg.nTimeRange, LOG_STR(strTimeFlg));
		}
		else
		{
			// 处理失败 todo
			m_hasCompleteSave_today[strTimeFlg] = false;
			LOG_(LOGID_STATION, LOG_F("[%s-%d][%s]时次站点数据处理失败,err=%d."), LOG_STR(m_stStationCfg.strCityType), m_stStationCfg.nTimeRange, LOG_STR(strTimeFlg), nErr);
		}
...
/* 是否时间已到                                                         */
/************************************************************************/
bool TaskStationMerge::IsTimeOut(int nHour, int nMinute)
{
...
/* 保存数据处理                                                         */
/************************************************************************/
int TaskStationMerge::SaveProcess(ST_REPORTTIME stForecast, int nYear, int nMonth, int nDay, int nHour)
{
	...
	// 北京时-国际时 转换
	// nHour -= 8;
	QDateTime tUTime = HandleCommon::GetUTC(nYear, nMonth, nDay, nHour, 0, 0);
	nYear = tUTime.date().year();
	nMonth = tUTime.date().month();
	nDay = tUTime.date().day();
	nHour = tUTime.time().hour();
	...
	if (nStationCnt != 0)
	{
		...
		if (HandleCommon::HandleGribFolder(strPath))
		{
			...
			{
				// 5、 数据分发
				DataDistribution(stForecast.stSaveFile.lstDDS, strFileName, strPath);
			}
		}
	}
	// 6、 释放存储空间
...
/* 获取要保存的站点信息                                                 */
/************************************************************************/
int TaskStationMerge::GetStationData(char * szStationNo, QString strSaveRule, int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond, float * &fData, int &nRow, int &nCol)
{
	// 1、 初始化
	fData = NULL;
	nRow = 0;
	...
	for (nForecastTime = m_stStationCfg.nTimeRange; nForecastTime <= m_stStationCfg.nMaxForecastTime; nForecastTime += m_stStationCfg.nTimeRange)
	{
		...
		nErr = nwfd_get_stationdata_forecasttime(m_stStationCfg.strCityType.toLocal8Bit().data(), strSaveRule.toLocal8Bit().data(), m_stStationCfg.nTimeRange,
			szStationNo, year, month, day, hour, 0, 0, fForecastData, stDataInfo, nCnt);
...
/* 从缓存中获取站点数据方法                                             */
/************************************************************************/
int TaskStationMerge::nwfd_get_stationdata_forecasttime(const char * strCityType, const char * szRule, int nTimeRange, const char * szStationNo,
	int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond, float * &fData, nwfd_station_data *& stDataInfo, int &nCnt)
{
	...
	for (iterStation = lstStationNo.begin(); iterStation != lstStationNo.end(); iterStation++)
	{
		...
		for (iterRule = lstRule.begin(); iterRule != lstRule.end(); iterRule++)
		{
			...
			// 获取信息
			//nErrCode = m_pCachedImpl->GeStationInfo(strCityType, strRule.data(), nTimeRange, nStationNo, fLon, fLat, fHeight);
			//if (nErrCode != SUCCESS)
			//{
			//	// 获取信息失败，跳过
			//	continue;
...
/* 计算预报时间                                                         */
/************************************************************************/
time_t TaskStationMerge::GetTime(int year, int month, int day, int hour, int minute, int second, int forecast)
{
...
/* 获取预报时间                                                         */
/************************************************************************/
void TaskStationMerge::GetForecastTime(int nYear, int nMonth, int nDay, int nHour, int nForecastTime, int &year, int &month, int &day, int &hour)
{
...
/* 获取要保存的站点信息                                                 */
/************************************************************************/
int TaskStationMerge::GetStationList(QString strSaveStation, LIST_STATION_INFO &lstStationInfo)
{
	// 打开文件
	FILE *fp;
...
/* 是否已经处理判断方法                                                 */
/************************************************************************/
bool TaskStationMerge::IsHasProcessed(QString strFile)
{
	// 查看今天处理的数据
...
/* 是否已经保存过的时效                                                 */
/************************************************************************/
bool TaskStationMerge::IsHasSaved(QString strTime)
{
	// 查看今天处理的数据
...
/* 已处理产品Hash标记维护处理                                           */
/************************************************************************/
void TaskStationMerge::HandleClearHash()
{
	// 按天进行标记判断处理
...
/* 数据分发处理                                                         */
/************************************************************************/
int TaskStationMerge::DataDistribution(QStringList lstDDS, QString strFileName, QString strPath)
{
