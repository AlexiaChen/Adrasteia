...
FTP_API CFTP::Put(const std::string &strRemoteFile, const std::string &strLocalFile)
{
    ...
    // 传输数据完毕，获取传送数据大小
    long nSize = getFileLength(strRemoteFileTmp);
...
FTP_API CFTP::downLoad(const std::string &strRemoteFile, const std::string &strLocalFile, const int pos, const unsigned int length)
{
    ...
    std::string strCmdLine = parseCommand(FTP_COMMAND_DOWNLOAD_FILE, strRemoteFile);
...
FTP_API CFTP::downLoad2(const std::string &strRemoteFile, const std::string& strLocalFile, const int pos /*= 0*/, const unsigned int length /*= 0*/)
{
    ...
    file = createLocalFile2(std::string(strLocalFile));
    assert(file != NULL);
...
FILE * CFTP::createLocalFile2(const std::string & strLocalFile)
{
    return fopen(strLocalFile.c_str(), "ab+");
}
