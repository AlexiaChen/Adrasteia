#ifndef NWFDTHREADHANDLE_H
#define NWFDTHREADHANDLE_H
...
// 默认使用的是控制总线程数。每个现成可处理10个socket。

class NwfdThreadHandle
{
public:
	~NwfdThreadHandle();
	...
	static NwfdThreadHandle & getClass(); //返回对象引用，是单例类
...
private:
	...
	typedef QHash<QThread *, unsigned int>  HASH_THREAD;
	HASH_THREAD threadSize;//保存每个线程的数目
	bool initfist;//是否是第一次初始化，只允许初始化一次。

};

#endif // NWFDTHREADHANDLE_H
