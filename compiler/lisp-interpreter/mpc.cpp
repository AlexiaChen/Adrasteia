...
static mpc_state_t *mpc_state_copy(mpc_state_t s) {
    mpc_state_t *r = (mpc_state_t *)malloc(sizeof(mpc_state_t));
...
static mpc_err_t *mpc_err_new(const char *filename, mpc_state_t s, const char *expected, char recieved) {
    mpc_err_t *x = (mpc_err_t *)malloc(sizeof(mpc_err_t));
    x->filename = (char*)malloc(strlen(filename) + 1);
    strcpy(x->filename, filename);
    x->state = s;
    x->expected_num = 1;
    x->expected = (char**)malloc(sizeof(char*));
    x->expected[0] = (char*)malloc(strlen(expected) + 1);
...
static mpc_err_t *mpc_err_fail(const char *filename, mpc_state_t s, const char *failure) {
    ...
    x->expected_num = 0;
    x->expected = NULL;
    x->failure = (char*)malloc(strlen(failure) + 1);
...
static void mpc_err_add_expected(mpc_err_t *x, char *expected) {
    ...
    x->expected_num++;
    x->expected = (char**)realloc(x->expected, sizeof(char*) * x->expected_num);
    x->expected[x->expected_num - 1] = (char*)malloc(strlen(expected) + 1);
...
static mpc_err_t *mpc_err_or(mpc_err_t** x, int n) {
    ...
    e->expected = NULL;
    e->failure = NULL;
    e->filename = (char*)malloc(strlen(x[0]->filename) + 1);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (x[i]->failure) {
            e->failure = (char*)malloc(strlen(x[i]->failure) + 1);
...
static mpc_err_t *mpc_err_repeat(mpc_err_t *x, const char *prefix) {
    ...
    if (x->expected_num == 1) {
        expect = (char*)realloc(expect, strlen(expect) + strlen(x->expected[0]) + 1);
    ...
    if (x->expected_num > 1) {
        ...
        for (i = 0; i < x->expected_num - 2; i++) {
            expect = (char*)realloc(expect, strlen(expect) + strlen(x->expected[i]) + strlen(", ") + 1);
            strcat(expect, x->expected[i]);
            strcat(expect, ", ");
        }

        expect = (char*)realloc(expect, strlen(expect) + strlen(x->expected[x->expected_num - 2]) + strlen(" or ") + 1);
        strcat(expect, x->expected[x->expected_num - 2]);
        strcat(expect, " or ");
        expect = (char*)realloc(expect, strlen(expect) + strlen(x->expected[x->expected_num - 1]) + 1);
...
static mpc_input_t *mpc_input_new_string(const char *filename, const char *string) {
    ...
    i->filename = (char*)malloc(strlen(filename) + 1);
    strcpy(i->filename, filename);
    i->type = MPC_INPUT_STRING;

    i->state = mpc_state_new();

    i->string = (char*)malloc(strlen(string) + 1);
...
static void mpc_input_mark(mpc_input_t *i) {
    ...
    i->marks_num++;
    i->marks = (mpc_state_t*)realloc(i->marks, sizeof(mpc_state_t) * i->marks_num);
    i->lasts = (char*)realloc(i->lasts, sizeof(char) * i->marks_num);
    i->marks[i->marks_num - 1] = i->state;
    i->lasts[i->marks_num - 1] = i->last;

    if (i->type == MPC_INPUT_PIPE && i->marks_num == 1) {
        i->buffer = (char*)calloc(1, 1);
    }
...
static int mpc_input_success(mpc_input_t *i, char c, char **o) {
    ...
    if (i->type == MPC_INPUT_PIPE &&
        ...
        i->buffer = (char*)realloc(i->buffer, strlen(i->buffer) + 2);
...
static mpc_stack_t *mpc_stack_new(const char *filename) {
    mpc_stack_t *s = (mpc_stack_t *)malloc(sizeof(mpc_stack_t));
...
static void mpc_stack_parsers_reserve_more(mpc_stack_t *s) {
    if (s->parsers_num > s->parsers_slots) {
        ...
        s->parsers = (mpc_parser_t**)realloc(s->parsers, sizeof(mpc_parser_t*) * s->parsers_slots);
        s->states = (int*)realloc(s->states, sizeof(int) * s->parsers_slots);
    }
}
...
static void mpc_stack_results_reserve_more(mpc_stack_t *s) {
    if (s->results_num > s->results_slots) {
        ...
        s->results = (mpc_result_t*)realloc(s->results, sizeof(mpc_result_t) * s->results_slots);
        s->returns = (int*)realloc(s->returns, sizeof(int) * s->results_slots);
    }
}
...
static void mpc_soft_delete(mpc_val_t *x) {
    mpc_undefine_unretained((mpc_parser_t*)x, 0);
}
...
void mpc_cleanup(int n, ...) {
    int i;
    mpc_parser_t **list = (mpc_parser_t **)malloc(sizeof(mpc_parser_t*) * n);
...
mpc_parser_t *mpc_oneof(const char *s) {
    ...
    p->data.string.x = (char*)malloc(strlen(s) + 1);
...
mpc_parser_t *mpc_or(int n, ...) {
    ...
    p->type = MPC_TYPE_OR;
    p->data. or .n = n;
    p->data. or .xs = (mpc_parser_t**)malloc(sizeof(mpc_parser_t*) * n);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
    ...
    p->data.and.n = n;
    p->data.and.f = f;
    p->data.and.xs = (mpc_parser_t**)malloc(sizeof(mpc_parser_t*) * n);
    p->data.and.dxs = (mpc_dtor_t*)malloc(sizeof(mpc_dtor_t) * (n - 1));
...
static mpc_val_t *mpcf_re_repeat(int n, mpc_val_t **xs) {
    ...
    (void)n;
    if (xs[1] == NULL) { return xs[0]; }
    if (strcmp((char*)xs[1], "*") == 0) { free(xs[1]); return mpc_many(mpcf_strfold, (mpc_parser_t*)xs[0]); }
    if (strcmp((char*)xs[1], "+") == 0) { free(xs[1]); return mpc_many1(mpcf_strfold, (mpc_parser_t*)xs[0]); }
    if (strcmp((char*)xs[1], "?") == 0) { free(xs[1]); return mpc_maybe_lift((mpc_parser_t*)xs[0], mpcf_ctor_str); }
    num = *(int*)xs[1];
    free(xs[1]);

    return mpc_count(num, mpcf_strfold, (mpc_parser_t*)xs[0], free);
}
...
static mpc_val_t *mpcf_re_range(mpc_val_t *x) {
    ...
    const char *s = (char*)x;
    int comp = s[0] == '^' ? 1 : 0;
    char *range = (char*)calloc(1, 1);

    if (s[0] == '\0') { free(x); return mpc_fail("Invalid Regex Range Expression"); }
    if (s[0] == '^' &&
        s[1] == '\0') {
        free(x); return mpc_fail("Invalid Regex Range Expression");
    }
    ...
    for (i = comp; i < strlen(s); i++) {
        ...
        if (s[i] == '\\') {
            ...
            if (tmp != NULL) {
                range = (char*)realloc(range, strlen(range) + strlen(tmp) + 1);
                strcat(range, tmp);
            }
            else {
                range = (char*)realloc(range, strlen(range) + 1 + 1);
        ...
        /* Regex Range...Range */
            ...
            if (s[i + 1] == '\0' || i == 0) {
                range = (char*)realloc(range, strlen(range) + strlen("-") + 1);
                strcat(range, "-");
            }
            else {
                start = s[i - 1] + 1;
...
static mpc_val_t *mpcf_escape_new(mpc_val_t *x, const char *input, const char **output) {
    ...
    while (*s) {
        ...
        while (output[i]) {
            if (*s == input[i]) {
                y = (char*)realloc(y, strlen(y) + strlen(output[i]) + 1);
                strcat(y, output[i]);
                found = 1;
                break;
            }
...
mpc_val_t *mpcf_strfold(int n, mpc_val_t **xs) {
    ...
    for (i = 0; i < n; i++) {
        x = (char*)realloc(x, strlen(x) + strlen((char*)xs[i]) + 1);
        strcat(x, (char*)xs[i]);
        free(xs[i]);
    }
    return x;
...
mpc_val_t *mpcf_maths(int n, mpc_val_t **xs) {
    ...
    if (strcmp((char*)xs[1], "*") == 0) { *vs[0] *= *vs[2]; }
    if (strcmp((char*)xs[1], "/") == 0) { *vs[0] /= *vs[2]; }
    if (strcmp((char*)xs[1], "%") == 0) { *vs[0] %= *vs[2]; }
    if (strcmp((char*)xs[1], "+") == 0) { *vs[0] += *vs[2]; }
    if (strcmp((char*)xs[1], "-") == 0) { *vs[0] -= *vs[2]; }
...
mpc_ast_t *mpc_ast_new(const char *tag, const char *contents) {
    ...
    a->contents = (char*)malloc(strlen(contents) + 1);
...
mpc_ast_t *mpc_ast_add_child(mpc_ast_t *r, mpc_ast_t *a) {
    r->children_num++;
    r->children = (mpc_ast_t**)realloc(r->children, sizeof(mpc_ast_t*) * r->children_num);
...
mpc_ast_t *mpc_ast_add_tag(mpc_ast_t *a, const char *t) {
    if (a == NULL) { return a; }
    a->tag = (char*)realloc(a->tag, strlen(t) + 1 + strlen(a->tag) + 1);
...
mpc_ast_t *mpc_ast_tag(mpc_ast_t *a, const char *t) {
    a->tag = (char*)realloc(a->tag, strlen(t) + 1);
    strcpy(a->tag, t);
    return a;
}
...
mpc_val_t *mpcf_str_ast(mpc_val_t *c) {
    mpc_ast_t *a = mpc_ast_new("", (char*)c);
    free(c);
    return a;
}
...
static mpc_val_t *mpcaf_grammar_repeat(int n, mpc_val_t **xs) {
    ...
    (void)n;
    if (xs[1] == NULL) { return xs[0]; }
    if (strcmp((char*)xs[1], "*") == 0) { free(xs[1]); return mpca_many((mpc_parser_t*)xs[0]); }
    if (strcmp((char*)xs[1], "+") == 0) { free(xs[1]); return mpca_many1((mpc_parser_t*)xs[0]); }
    if (strcmp((char*)xs[1], "?") == 0) { free(xs[1]); return mpca_maybe((mpc_parser_t*)xs[0]); }
    if (strcmp((char*)xs[1], "!") == 0) { free(xs[1]); return mpca_not((mpc_parser_t*)xs[0]); }
    num = *((int*)xs[1]);
    free(xs[1]);
    return mpca_count(num, (mpc_parser_t*)xs[0]);
}

static mpc_val_t *mpcaf_grammar_string(mpc_val_t *x, void *s) {
    mpca_grammar_st_t *st = (mpca_grammar_st_t *)s;
    char *y = (char*)mpcf_unescape(x);
...
static mpc_val_t *mpcaf_grammar_regex(mpc_val_t *x, void *s) {
    mpca_grammar_st_t *st = (mpca_grammar_st_t *)s;
    char *y = (char*)mpcf_unescape_regex(x);
...
static mpc_parser_t *mpca_grammar_find_parser(char *x, mpca_grammar_st_t *st) {
    ...
    if (is_number(x)) {
        ...
        while (st->parsers_num <= i) {
            st->parsers_num++;
            st->parsers = (mpc_parser_t**)realloc(st->parsers, sizeof(mpc_parser_t*) * st->parsers_num);
...
static mpc_val_t *mpcaf_grammar_id(mpc_val_t *x, void *s) {
    ...
    mpca_grammar_st_t *st = (mpca_grammar_st_t *)s;
    mpc_parser_t *p = mpca_grammar_find_parser((char*)x, st);
    free(x);
...
mpc_parser_t *mpca_grammar_st(const char *grammar, mpca_grammar_st_t *st) {
    ...
    return (st->flags & MPCA_LANG_PREDICTIVE) ? mpc_predictive((mpc_parser_t *)r.output) : (mpc_parser_t *)r.output;
...
static mpc_val_t *mpca_stmt_afold(int n, mpc_val_t **xs) {
    mpca_stmt_t *stmt = (mpca_stmt_t *)malloc(sizeof(mpca_stmt_t));
...
static mpc_val_t *mpca_stmt_fold(int n, mpc_val_t **xs) {
    ...
    int i;
    mpca_stmt_t **stmts = (mpca_stmt_t **)malloc(sizeof(mpca_stmt_t*) * (n + 1));

    for (i = 0; i < n; i++) {
        stmts[i] = (mpca_stmt_t *)xs[i];
    }
    stmts[n] = NULL;
